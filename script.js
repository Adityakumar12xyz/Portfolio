...
(function () {
    ...
    function buildDOM() {
        ...
        panel.innerHTML = `
          ...
          <div class="cc-section">
            ...
            <div class="cc-themes">
              <div class="cc-theme-opt" data-theme="quantum"><span class="cc-color-dot quantum"></span>Eco Forest 🌿</div>
    ...
    // ── AUTO COLOR CYCLING (30 colors in 1 hour = every 2 min) ──
    const AUTO_COLORS = [
        { primary:'#10b981', rgb:'16,185,129',  accent:'#34d399', bg:'#07110c', surface:'#0f2219', glow:'rgba(16,185,129,0.22)' },
        { primary:'#06b6d4', rgb:'6,182,212',   accent:'#67e8f9', bg:'#051b20', surface:'#0a2830', glow:'rgba(6,182,212,0.22)' },
        { primary:'#8b5cf6', rgb:'139,92,246',  accent:'#a78bfa', bg:'#0c0720', surface:'#150e30', glow:'rgba(139,92,246,0.22)' },
        { primary:'#f43f5e', rgb:'244,63,94',   accent:'#fb7185', bg:'#1a0510', surface:'#2a0c1a', glow:'rgba(244,63,94,0.22)' },
        { primary:'#f59e0b', rgb:'245,158,11',  accent:'#fcd34d', bg:'#120d02', surface:'#1f1805', glow:'rgba(245,158,11,0.22)' },
        { primary:'#ec4899', rgb:'236,72,153',  accent:'#f9a8d4', bg:'#1a0515', surface:'#2a0c22', glow:'rgba(236,72,153,0.22)' },
        { primary:'#14b8a6', rgb:'20,184,166',  accent:'#5eead4', bg:'#05110f', surface:'#0a221e', glow:'rgba(20,184,166,0.22)' },
        { primary:'#e879f9', rgb:'232,121,249', accent:'#f0abfc', bg:'#15051a', surface:'#220c2a', glow:'rgba(232,121,249,0.22)' },
        { primary:'#22c55e', rgb:'34,197,94',   accent:'#86efac', bg:'#020d05', surface:'#061a0c', glow:'rgba(34,197,94,0.22)' },
        { primary:'#3b82f6', rgb:'59,130,246',  accent:'#93c5fd', bg:'#050a1a', surface:'#0c1530', glow:'rgba(59,130,246,0.22)' },
        { primary:'#ef4444', rgb:'239,68,68',   accent:'#fca5a5', bg:'#1a0505', surface:'#2a0c0c', glow:'rgba(239,68,68,0.22)' },
        { primary:'#a855f7', rgb:'168,85,247',  accent:'#c084fc', bg:'#0f0520', surface:'#1a0c30', glow:'rgba(168,85,247,0.22)' },
        { primary:'#84cc16', rgb:'132,204,22',  accent:'#bef264', bg:'#0a0f02', surface:'#141f05', glow:'rgba(132,204,22,0.22)' },
        { primary:'#f97316', rgb:'249,115,22',  accent:'#fdba74', bg:'#1a0c02', surface:'#2a1505', glow:'rgba(249,115,22,0.22)' },
        { primary:'#06d6a0', rgb:'6,214,160',   accent:'#6ee7b7', bg:'#021510', surface:'#052a20', glow:'rgba(6,214,160,0.22)' },
        { primary:'#7c3aed', rgb:'124,58,237',  accent:'#a78bfa', bg:'#08031a', surface:'#120530', glow:'rgba(124,58,237,0.22)' },
        { primary:'#fb923c', rgb:'251,146,60',  accent:'#fed7aa', bg:'#1a0e05', surface:'#2a1a0c', glow:'rgba(251,146,60,0.22)' },
        { primary:'#2dd4bf', rgb:'45,212,191',  accent:'#99f6e4', bg:'#021512', surface:'#052a24', glow:'rgba(45,212,191,0.22)' },
        { primary:'#c026d3', rgb:'192,38,211',  accent:'#e879f9', bg:'#15021a', surface:'#22052a', glow:'rgba(192,38,211,0.22)' },
        { primary:'#16a34a', rgb:'22,163,74',   accent:'#4ade80', bg:'#020f06', surface:'#051f0f', glow:'rgba(22,163,74,0.22)' },
        { primary:'#0ea5e9', rgb:'14,165,233',  accent:'#7dd3fc', bg:'#020f18', surface:'#051f30', glow:'rgba(14,165,233,0.22)' },
        { primary:'#d946ef', rgb:'217,70,239',  accent:'#e879f9', bg:'#18031a', surface:'#28052a', glow:'rgba(217,70,239,0.22)' },
        { primary:'#65a30d', rgb:'101,163,13',  accent:'#a3e635', bg:'#080e02', surface:'#121f05', glow:'rgba(101,163,13,0.22)' },
        { primary:'#e11d48', rgb:'225,29,72',   accent:'#fb7185', bg:'#1a0208', surface:'#2a0512', glow:'rgba(225,29,72,0.22)' },
        { primary:'#0891b2', rgb:'8,145,178',   accent:'#22d3ee', bg:'#020e14', surface:'#051e28', glow:'rgba(8,145,178,0.22)' },
        { primary:'#9333ea', rgb:'147,51,234',  accent:'#c084fc', bg:'#0a031a', surface:'#15052a', glow:'rgba(147,51,234,0.22)' },
        { primary:'#ea580c', rgb:'234,88,12',   accent:'#fb923c', bg:'#1a0802', surface:'#2a1205', glow:'rgba(234,88,12,0.22)' },
        { primary:'#059669', rgb:'5,150,105',   accent:'#34d399', bg:'#020f0a', surface:'#051f15', glow:'rgba(5,150,105,0.22)' },
        { primary:'#4f46e5', rgb:'79,70,229',   accent:'#818cf8', bg:'#050420', surface:'#0c0a30', glow:'rgba(79,70,229,0.22)' },
        { primary:'#dc2626', rgb:'220,38,38',   accent:'#f87171', bg:'#1a0303', surface:'#2a0808', glow:'rgba(220,38,38,0.22)' }
    ];
    let autoColorIndex = 0;
    let autoColorInterval = null;

    function applyAutoColor(idx) {
        const c = AUTO_COLORS[idx % AUTO_COLORS.length];
        const root = document.documentElement;
        root.style.setProperty('--primary', c.primary);
        root.style.setProperty('--primary-rgb', c.rgb);
    ...
    class Dot {
        ...
        reset() {
            ...
            this.angleSpeed = (Math.random() - .5) * 0.015;
        }
        update() {
        ...
        draw(col) {
            ...
            // Draw a small stylized leaf shape
            ctx.moveTo(0, this.r * 3.5);
            ctx.quadraticCurveTo(this.r * 2.2, 0, 0, -this.r * 3.5);
            ctx.quadraticCurveTo(-this.r * 2.2, 0, 0, this.r * 3.5);
            
            // Draw a tiny stem at the base
            ctx.moveTo(0, this.r * 3.5);
            ...
            // Soft leaf border for premium look
    ...
    // ── CERTIFICATE & OFFER LETTER MODAL (with iframe for PDFs) ──
    function initCertModal() {
        const modal = $('cert-modal');
        const modalImg = $('cert-modal-img');
        const modalIframe = $('cert-modal-iframe');
        const modalTitle = $('cert-modal-title');
        const modalInfo = $('cert-modal-info');
        const closeBtn = $('cert-modal-close');
        
        if (!modal || !closeBtn) return;
        
        // Open Modal for both cert-action-btn and offer-action-btn
        $$('.cert-action-btn, .offer-action-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                // If PDF, show iframe; otherwise show image
                if (pdfSrc) {
                ...
                if (btn.classList.contains('offer-action-btn')) {
                    unlock('viewOffer','Offer Reviewer 📋','Viewed an official offer letter.');
                } else {
                    unlock('viewCert','Credential Inspector 🔍','Inspected an official certificate in high-res.');
                }
            });
        });
        ...
        modal.addEventListener('click', e => {
            if (e.target === modal || e.target.classList.contains('cert-modal-glow')) {
                closeModal();
            }
        });
    ...
    // ── CONTENT PROTECTION (anti-screenshot/share/intercept) ─────
    function initContentProtection() {
        const protectedSelectors = '.cert-img-wrap, .cert-modal-content, .offer-card, #cert-modal, .offer-img-wrap';
        
        // Disable right-click on protected content
        document.addEventListener('contextmenu', e => {
            if (e.target.closest(protectedSelectors)) {
                e.preventDefault();
                toast('Protected Content 🔒', 'This content is protected and cannot be copied.', true);
            }
        });
        ...
        document.addEventListener('keydown', e => {
            if (e.key === 'PrintScreen') {
                e.preventDefault();
                toast('Screenshot Blocked 🛡️', 'Certificates & offer letters are protected.', true);
            }
            // Block Ctrl+Shift+S (save as), Ctrl+P (print), Ctrl+Shift+I (dev tools)
            if ((e.ctrlKey && e.shiftKey && (e.key === 'S' || e.key === 's' || e.key === 'I' || e.key === 'i')) ||
                (e.ctrlKey && (e.key === 'p' || e.key === 'P'))) {
                const modal = $('cert-modal');
                if (modal && modal.classList.contains('active')) {
                    e.preventDefault();
                    toast('Action Blocked 🔒', 'Cannot print/save protected content.', true);
                }
            }
        });
        
        // Blur content on visibility change (tab switch for screenshot apps)
        document.addEventListener('visibilitychange', () => {
            const modal = $('cert-modal');
            const protectedEls = $$('.cert-img-wrap img, .offer-img-wrap img, .offer-img-wrap iframe');
            if (document.hidden) {
                protectedEls.forEach(el => el.style.filter = 'blur(20px)');
                if (modal && modal.classList.contains('active')) {
                    const mImg = $('cert-modal-img');
                    const mIf = $('cert-modal-iframe');
                    if (mImg) mImg.style.filter = 'blur(20px)';
                    if (mIf) mIf.style.filter = 'blur(20px)';
                }
            } else {
                protectedEls.forEach(el => el.style.filter = '');
                if (modal) {
                    const mImg = $('cert-modal-img');
                    const mIf = $('cert-modal-iframe');
                    if (mImg) mImg.style.filter = '';
                    if (mIf) mIf.style.filter = '';
                }
            }
        });
    }
